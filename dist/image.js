...
const debug = (0, debug_1.default)('cordova-res:image');
/**
...
async function resolveSourceImage(platform, type, sources, errstream) {
    ...
    const msg = util_1.default.format(`Missing valid source image for %s %s (sources: %s)`, (0, platform_1.prettyPlatform)(platform), (0, resources_1.prettyResourceType)(type, { pluralize: true }), sources.join(', '));
...
async function readSourceImage(platform, type, src, errstream) {
    const image = (0, sharp_1.default)(await (0, utils_fs_1.readFile)(src));
    const metadata = await (0, resources_1.validateResource)(platform, type, src, image, errstream);
    debug('Source image for %s: %O', type, metadata);
    return {
        platform,
        resource: type,
        type: "raster" /* SourceType.RASTER */,
        src,
...
async function generateImage(image, src, metadata, errstream) {
    if (image.format === "none" /* Format.NONE */) {
    ...
    await (0, utils_fs_1.writeFile)(image.src, await pipeline.toBuffer());
}
exports.generateImage = generateImage;
async function applyTransformations(src, transformations) {
    return (0, utils_array_1.reduce)(transformations, async (pipeline, transformation) => transformation(pipeline), src);
}
